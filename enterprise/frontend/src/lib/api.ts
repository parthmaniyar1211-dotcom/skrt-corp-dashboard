...
// Add a response interceptor for error handling & virtual DB fallback
api.interceptors.response.use(
  (response) => response,
  (error) => {
    // If it's a network error (server is down or we are offline), run Virtual DB
    if (!error.response || error.code === 'ERR_NETWORK' || error.message === 'Network Error') {
      ...
      console.warn(`⚡ Virtual DB interceptor stepped in: ${method.toUpperCase()} ${endpoint}`);

      let responseData: any = { success: false, message: "Virtual DB not found" };

      // Helper to simulate Axios envelope
      const makeAxiosResponse = (data: any) => ({
        data,
        status: 200,
        statusText: "OK",
        headers: {},
      ...
      if (resource === "inventory") {
        ...
        return Promise.resolve(makeAxiosResponse(responseData));
      }
      ...
      // 3. TRACKING ENDPOINTS (Dynamically calculated based on shipments!)
      ...
      // 9. ANALYTICS DETAILED
      if (resource === "analytics" && parts[1] === "detailed") {
        const shipmentsList = getCollection("skrt_shipments", initialShipments);
        const invoicesList = getCollection("skrt_invoices", initialInvoices);
        const expensesList = getCollection("skrt_expenses", initialExpenses);
      ...
      if (resource === "analytics" && parts[1] === "dashboard") {
        const shipmentsList = getCollection("skrt_shipments", initialShipments);
        const invoicesList = getCollection("skrt_invoices", initialInvoices);
        const vehiclesList = getCollection("skrt_vehicles", initialVehicles);
    ...
    // Default error handling for regular backend responses
    if (error.response) {
